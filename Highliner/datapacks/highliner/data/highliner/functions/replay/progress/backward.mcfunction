...
scoreboard players operation !turn_difference constants = !turn constants
scoreboard players operation !turn_difference constants -= !target_turn constants
execute if score !turn_difference constants matches 2.. unless score !target_turn constants = !turn constants run tellraw @a {"translate":"highliner.message.replay.announce_movement","color":"yellow","italic":true,"with":[{"score":{"objective":"constants","name":"!turn"}},{"score":{"objective":"constants","name":"!target_turn"}}]}
