...
execute if score !gamemode constants matches 3 run item replace entity @s[tag=replayer] hotbar.1 with minecraft:golden_chestplate{display: {Name: '{"translate":"highliner.item.name.replay.next_turn","with":[{"score":{"objective":"constants","name":"!replay_jump_speed"}},{"keybind":"key.use"}]}'}}
